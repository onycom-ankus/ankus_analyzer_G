...
public class Load_Engineinfo extends HttpServlet {
	...
	public void doGet(HttpServletRequest req, HttpServletResponse resp) throws IOException {
    	...
    	try{
			...
			rs = st.executeQuery("SELECT * FROM ENGINE WHERE ID =" + sReqid);
	        ...
	        if(rs.next())
	        {
		        ...
		        data.put("PORT", rs.getString("PORT"));
		        data.put("CLUSTER_ID", rs.getString("CLUSTER_ID"));
	        }
	        String cluster_id = rs.getString("CLUSTER_ID");
	        rs.close();
	        rs = st.executeQuery("SELECT * FROM ADMIN_HADOOP_CLUSTER WHERE ID =" + cluster_id);
			
	        if(rs.next())
	        {
		        data.put("CLUSTER_NAME", rs.getString("NAME"));
